...
trait Huffman extends HuffmanInterface {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def dfs(remaining: CodeTree, bits: List[Bit]): List[Char] = remaining match {
      case Leaf(char, weight) => if (bits.isEmpty) List(char) else (char :: dfs(tree, bits))
      case Fork(left, right, chars, weight) => if (bits.head == 0) dfs(left, bits.tail) else dfs(right, bits.tail)
    }
    dfs(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def lookup(tree: CodeTree)(c: Char): List[Bit] = tree match {
      case Leaf(c, w) => List()
      case Fork(left, right, chs, wt) => if (chars(left).contains(c)) 0 :: lookup(left)(c) else 1 :: lookup(right)(c)
    }
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    ...
    case Fork(left, right, chs, wt) => mergeCodeTables(convert(left), convert(right))
  }
